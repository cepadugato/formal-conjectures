...
theorem interval_block_run (m M : ℤ) (hm : 2 ≤ m) (hM : 2 * m ≤ M) :
  ...
  have hsplit : ∀ a b : ℤ, a ≤ b → Finset.Icc a b = insert a (Finset.Icc (a + 1) b) := by
    intro a b hab
    ext x
    simp only [Finset.mem_Icc, Finset.mem_insert]
    omega
  have key : ∀ j : ℕ, ∀ p : ℤ, m ≤ p → p ≤ M / 2 + 1 → M / 2 + 1 - p = (j : ℤ) →
      ∀ k : ℤ, M / 2 + 1 ≤ k → k ≤ M + ∑ v ∈ Finset.Icc p (M / 2), v →
        k ∈ subsetSums ((Finset.Icc p M : Finset ℤ) : Set ℤ) := by
    intro j
    induction j with
    | zero =>
      intro p hp₁ hp₂ hj k hk₁ hk₂
      have hpe : p = M / 2 + 1 := by omega
      subst hpe
      rw [Finset.Icc_eq_empty (by omega), Finset.sum_empty] at hk₂
      refine ⟨{k}, ?_, by simp⟩
      simp only [Finset.coe_singleton, Set.singleton_subset_iff, Finset.mem_coe, Finset.mem_Icc]
      omega
    | succ j ih =>
      intro p hp₁ hp₂ hj k hk₁ hk₂
      have hpM₂ : p ≤ M / 2 := by omega
      have hpM : p ≤ M := by omega
      have ihp := ih (p + 1) (by omega) (by omega) (by omega)
      have hnotmem : p ∉ Finset.Icc (p + 1) (M / 2) := by simp
      have hnotmem' : p ∉ Finset.Icc (p + 1) M := by simp
      have hsum : ∑ v ∈ Finset.Icc p (M / 2), v = p + ∑ v ∈ Finset.Icc (p + 1) (M / 2), v := by
        rw [hsplit p (M / 2) hpM₂, Finset.sum_insert hnotmem]
      have hsum_nonneg : 0 ≤ ∑ v ∈ Finset.Icc (p + 1) (M / 2), v :=
        Finset.sum_nonneg fun x hx => by
          have := Finset.mem_Icc.mp hx
          omega
      have hhalf : 2 * (M / 2) ≤ M := by omega
      rw [hsplit p M hpM]
      refine merge_fact (M / 2 + 1) (M + ∑ v ∈ Finset.Icc (p + 1) (M / 2), v)
        (Finset.Icc (p + 1) M) p ihp hnotmem' (by omega) k hk₁ ?_
      rw [hsum] at hk₂
      omega
  intro k hk₁ hk₂
  exact key (M / 2 + 1 - m).toNat m le_rfl (by omega) (by omega) k hk₁ hk₂
...
theorem isGoodPair_of_two_le_of_window (t α : ℝ) (M : ℤ)
  ...
  have hα32 : α < 3 / 2 := by
    ...
    linarith
  have hex : ∃ n : ℕ, M < ⌊t * α ^ n⌋ := by
    obtain ⟨n, hn⟩ := floorSeq_unbounded t α ht0 hα1 (M + 1); exact ⟨n, by omega⟩
  obtain ⟨N₀, hN₀, hN₀min⟩ : ∃ N₀ : ℕ, M < ⌊t * α ^ N₀⌋ ∧ ∀ j, j < N₀ → ⌊t * α ^ j⌋ ≤ M :=
    ⟨Nat.find hex, Nat.find_spec hex, fun j hj => by have := Nat.find_min hex hj; omega⟩
  have himg : (Finset.range N₀).image (fun n : ℕ => ⌊t * α ^ n⌋) = Finset.Icc ⌊t⌋ M :=
    floorSeq_image_range_cut t α M ht0 hM1 hα1 hα N₀ hN₀ hN₀min
  have hcut : ⌊t * α ^ N₀⌋ ≤ M + 2 := by
    obtain ⟨p, rfl⟩ : ∃ p : ℕ, N₀ = p + 1 := by
      cases N₀ with
      | zero => rw [floorSeq_zero] at hN₀; omega
      | succ p => exact ⟨p, rfl⟩
    exact floorSeq_le_add_two_at_cut t α M p ht0.le hM1 hα1 hα (hN₀min p (Nat.lt_succ_self p))
  have hS1 : M / 2 + 2 ≤ ∑ v ∈ Finset.Icc ⌊t⌋ (M / 2), v := by
    have hpair : ({M / 2 - 1, M / 2} : Finset ℤ) ⊆ Finset.Icc ⌊t⌋ (M / 2) := by
      intro x hx
      simp only [Finset.mem_insert, Finset.mem_singleton] at hx
      rw [Finset.mem_Icc]
      rcases hx with rfl | rfl <;> omega
    have hne : M / 2 - 1 ∉ ({M / 2} : Finset ℤ) := by simp
    have hsum : ∑ v ∈ ({M / 2 - 1, M / 2} : Finset ℤ), v = (M / 2 - 1) + M / 2 := by
      rw [Finset.sum_insert hne, Finset.sum_singleton]
    have hle : ∑ v ∈ ({M / 2 - 1, M / 2} : Finset ℤ), v ≤ ∑ v ∈ Finset.Icc ⌊t⌋ (M / 2), v := by
      apply Finset.sum_le_sum_of_subset_of_nonneg hpair
      intro x hx _
      rw [Finset.mem_Icc] at hx
      omega
    omega
  have hnew : ⌊t * α ^ N₀⌋ ∉ (Finset.range N₀).image (fun n : ℕ => ⌊t * α ^ n⌋) := by
    intro hmem
    rw [himg, Finset.mem_Icc] at hmem
    omega
  refine isGoodPair_of_run t α (by linarith) hα1 hα32 N₀ (M / 2 + 1)
    (M + ∑ v ∈ Finset.Icc ⌊t⌋ (M / 2), v) hnew ?_ ?_
  · intro k hk1 hk2
    rw [himg]
    exact hwin k hk1 hk2
  · omega
